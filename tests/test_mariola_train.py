...
def mock_dependencies():
    with mock.patch("mariola_train.get_parsed_arguments") as mock_get_args, mock.patch(
        "mariola_train.initialize_logger"
    ) as mock_initialize_logger, mock.patch(
        "mariola_train.log"
    ) as mock_log, mock.patch(
        "mariola_train.extract_settings_data"
    ) as mock_extract_settings, mock.patch(
        "mariola_train.load_data_from_csv"
    ) as mock_load_data, mock.patch(
        "mariola_train.normalize_df"
    ) as mock_normalize, mock.patch(
        "mariola_train.handle_pca"
    ) as mock_handle_pca, mock.patch(
        "mariola_train.create_sequences"
    ) as mock_create_sequences, mock.patch(
        "mariola_train.train_test_split"
    ) as mock_train_test_split, mock.patch(
        "mariola_train.Sequential"
    ) as mock_sequential, mock.patch(
        "mariola_train.model.fit"
    ) as mock_model_fit, mock.patch(
        "mariola_train.model.evaluate"
    ) as mock_model_evaluate, mock.patch(
        "mariola_train.save_df_info"
    ) as mock_save_info:
        ...
        yield mock_get_args, mock_initialize_logger, mock_log, mock_extract_settings, mock_load_data, mock_normalize, mock_handle_pca, mock_create_sequences, mock_train_test_split, mock_sequential, mock_model_fit, mock_model_evaluate, mock_save_info
