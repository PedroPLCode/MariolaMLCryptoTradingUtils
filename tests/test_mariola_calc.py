...
def mock_dependencies():
    with patch(
    ...
    ) as mock_get_parsed_arguments, patch(
        "utils.logger_utils.initialize_logger"
    ) as mock_initialize_logger, patch(
        "utils.logger_utils.log"
    ) as mock_log, patch(
        "utils.app_utils.extract_settings_data"
    ) as mock_extract_settings_data, patch(
        "utils.app_utils.load_data_from_csv"
    ) as mock_load_data_from_csv, patch(
        "utils.app_utils.save_data_to_csv"
    ) as mock_save_data_to_csv, patch(
        "utils.app_utils.save_df_info"
    ) as mock_save_df_info, patch(
        "utils.calc_utils.prepare_df"
    ) as mock_prepare_df:

        mock_get_parsed_arguments.return_value = (
        ...
        mock_extract_settings_data.return_value = {
            "settings": {"regresion": True, "clasification": False}
        }
...
def test_mariola_calc(mock_dependencies):
    ...
    mock_dependencies["mock_initialize_logger"].assert_called_with(
        "settings_filename.json"
    )

    mock_dependencies["mock_extract_settings_data"].assert_called_with(
        "settings_filename.json"
    )
    ...
    mock_dependencies["mock_save_data_to_csv"].assert_called_with(
        mock_dependencies["mock_prepare_df"](), "data_filename_calculated"
    )
    mock_dependencies["mock_save_df_info"].assert_called_with(
        mock_dependencies["mock_prepare_df"](), "data_filename_calculated.info"
    )

    mock_dependencies["mock_log"].assert_any_call(
        "MariolaCryptoTradingBot. Calculating DataFrame process starting."
    )
    mock_dependencies["mock_log"].assert_any_call(
        "MariolaCryptoTradingBot. Load data from csv file."
    )

    mock_dependencies["mock_log"].assert_any_call(
        "MariolaCryptoTradingBot. Calculating Technical Analysis parameters completed."
    )
